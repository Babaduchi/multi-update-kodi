...
Multi-Update is a cross-platform maintenance utility for Kodi 21 Omega. After confirmation it performs up to nine ordered steps without restarting Kodi:
...
6. Safely purges all registered texture-cache entries through Kodi's JSON-RPC API, without deleting the live `Textures13.db` file or restarting Kodi. It then shows texture cleanup statistics and offers **Scan Libraries** or **Exit**.
7. Runs and waits for a complete video-library update scan.
8. Runs and waits for a complete music-library update scan.
9. Cleans both video and music libraries.

The first summary reports the cache and installation-package sizes deleted, PVR/EPG refresh result, and orphaned add-on data cleanup statistics. **Delete Textures** continues to step 6; **Exit** stops. The second summary reports texture entries removed and failures. **Scan Libraries** continues to steps 7-9; **Exit** stops.
