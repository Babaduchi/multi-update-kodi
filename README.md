...
3. Reloads every enabled PVR client to refresh its cached PVR and EPG data without deleting Kodi's open databases.
   - Automatically compares `special://profile/addon_data/` with all installed and disabled add-ons, then deletes data folders belonging to add-ons that are no longer installed.
